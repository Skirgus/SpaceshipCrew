...
## Промежуточный результат (зафиксировано: 2026-03-30)

### Что сделано

- **Пожары от реактора**:
  - до `75%` риска нет;
  - `75..100%` — вероятность растёт, но без гарантии;
  - `>100%` — пожар гарантирован, время до пожара уменьшается к `120%`.
- **Очаги пожара**:
  - модель через количество очагов (`FireHotspotCount`) и максимум (`MaxFireHotspots`);
  - `FireIntensity` зависит от числа очагов;
  - на каждый очаг спавнится отдельная `BP_Station_Fire`;
  - при тушении очаги и станции удаляются по одному.
- **Кислород**:
  - `OxygenLevel` — процент O2 в атмосфере (цель `21%`);
  - `OxygenReserve` + `bOxygenSupplyEnabled`;
  - при выключении подачи O2 новые очаги не возникают при `O2 <= 12%`;
  - при `O2 <= 12%` существующие очаги постепенно затухают.
- **ActionId для станций**:
  - реактор: `IncreaseReactorPower`, `DecreaseReactorPower` (и совместимость с `AdjustReactor`);
  - кислород: `EnableOxygenSupply`, `DisableOxygenSupply`, `ToggleOxygenSupply`.
- **HUD (`UShipStatusWidget`)**:
  - добавлены reactor state, alerts, fire hotspot text, oxygen reserve/supply;
  - добавлены `GetFireFrameOpacity()` и `GetFireFrameColor()` под красную рамку при пожаре.
- **Надёжность интерактива**:
  - в `AShipInteractableBase` добавлен `InteractionCollider` (блок `ECC_Visibility`) для стабильного наведения лучом.

### Что сделать дальше

1. **Закрыть визуал/UI в редакторе**:
   - донастроить `WBP_ShipStatus` по новому layout;
   - включить/проверить fullscreen fire frame и alert-блок.
2. **Добить спавн fire-станций**:
   - проверить на карте `Lvl_Ship`, что станции гарантированно спавнятся в разных местах и доступны для наведения.
3. **Отладка ботов**:
   - отдельно проверить анимацию движения ботов при `MoveToActor`.
4. **Геймплейный раунд 10–15 минут**:
   - проверить цепочку: перегруз реактора -> пожар -> отключение O2 -> затухание очагов.
...
## Checkpoint (2026-03-31): модульные комнаты и двери

### Что сделано

- Введена конфигурация корабля через ассеты:
  - `ShipConfigAsset` (модули, соединения, ресурсы),
  - `ShipModuleDefinition` / `ShipModuleCatalog`.
- Корабль теперь можно собирать из готовых модулей по конфигу:
  - `AShipGameMode` выбирает `SelectedShipConfig`,
  - при необходимости спавнит `AShipActor` в рантайме и применяет конфиг.
- Реализована валидация конфига:
  - обязательные модули: `Reactor`, `Engine`, `Bridge`, `Airlock`,
  - обязательные capability: `OxygenStorage`, `FuelStorage`,
  - ресурсы `OxygenReserve/FuelReserve > 0`,
  - связность `Reactor -> Bridge`.
- Модули визуализируются как room-based геометрия:
  - interior-модуль -> пол/потолок/стены,
  - non-interior модуль -> внешний техблок.
- По точкам соединения:
  - room<->room создаёт проход с дверью,
  - если соединения нет — стена,
  - если сосед без внутреннего объема — двери нет, остаётся стена.
- Добавлен `AShipDoorActor`; двери синхронизируются с `Bulkheads` **event-driven** (без тиков):
  - `UShipSystemsComponent::OnBulkheadsChanged`,
  - `AShipActor` подписывается и обновляет состояние дверей.
- Добавлено внутреннее освещение комнат (`PointLight` в interior-модулях).

### Что дальше

1. Довести геометрию от debug-кубов до модульных "комнатных" мешей (стены/углы/потолки).
2. Привязать управление дверьми к интеракциям экипажа (станция/кнопка на двери).
3. Проверить навигацию ботов при закрытии переборок и перестроение маршрутов.
4. После стабилизации — перейти к Фазе B (единый Interaction API модулей).
