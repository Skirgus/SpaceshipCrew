...
**Высота (важно):** в `Location` задай **Z** так, чтобы **центр капсулы** персонажа был **над** коллизией палубы (часто **Z ≈ 90–100** см в локальных координатах корабля, если «пол» у корня). Если поставить **0,0,0** при толстом `HullMesh`, капсула окажется **внутри меша** — старый спавн без настроек коллизии мог **не создать** пешку, оставалась только камера. В коде для спавна экипажа включено **`AdjustIfPossibleButAlwaysSpawn`**, но точки всё равно лучше держать **над** палубой.
