...
bool FShipModuleCellSizeSyncTest::RunTest(const FString& Parameters)
{
	...
	Def->Size = FVector(800.0f, 800.0f, 400.0f);
	...
	TestEqual(TEXT("OneByThreeSizeY"), static_cast<float>(Def->Size.Y), 2400.0f);
	TestEqual(TEXT("OneByThreeSizeX"), static_cast<float>(Def->Size.X), 800.0f);
